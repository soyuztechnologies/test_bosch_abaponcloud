...
*        CATCH cx_number_ranges.
  ...
  METHOD copyTravel.
    ...
    data: travels type table for create ZBSH_AB_travel\\Travel,
          bookings_cba type table for create ZBSH_AB_travel\\Travel\_Booking,
          booksuppl_cba type table for create ZBSH_AB_travel\\Booking\_BookingSupplement.
...
*    Return the total amount in mapped so the RAP will modify this data to DB
  ...
  METHOD validateHeaderData.
    ...
    LOOP AT lt_travel INTO DATA(ls_travel).
      ...
      IF ls_travel-begindate is initial OR ls_travel-EndDate is initial.  "begin_date must be in the future

        APPEND VALUE #( %tky        = ls_travel-%tky ) TO failed-travel.

        APPEND VALUE #( %tky = ls_travel-%tky
                        %msg = NEW /dmo/cm_flight_messages(
                                    textid   = value #( msgid = 'ZBSH_AB_MSG' msgno = '000' )
                                    severity = if_abap_behv_message=>severity-error )
            ...
            append value #(    %cid = cond #( when operation = if_abap_behv=>op-m-create then entity-%cid_ref )
                                      ...
                                      %msg = new /dmo/cm_flight_messages(
                                                                                              textid                = value #(
                                                                                                                                     msgid = 'SY'
                                                                                                                                     msgno = 499
                                                                                                                                     attr1 = 'The country codes for agency '
                                                                                                                                     attr2 = 'and customer not matching'
                                                                                                                                  )
