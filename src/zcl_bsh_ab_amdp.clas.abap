CLASS zcl_bsh_ab_amdp DEFINITION
 PUBLIC
 FINAL
 CREATE PUBLIC .
 PUBLIC SECTION.
  ...
  CLASS-METHODS get_total_sales
               for table FUNCTION ZBSH_AB_TF_SALES.


       CLASS-METHODS get_product_mrp AMDP OPTIONS CDS SESSION CLIENT DEPENDENT READ-ONLY
                            IMPORTING value(i_tax) type i
                            EXPORTING
                                       value(otab) type ZBSH_AB_TT_MRP.
                                       CLASS-METHODS execute for  scalar function zbsh_ab_sf.
...
CLASS ZCL_BSH_AB_AMDP IMPLEMENTATION.
...
*        output =
    ...
    zcl_bsh_ab_amdp=>get_product_mrp(
     EXPORTING
       i_tax = 18
     IMPORTING
       otab  = data(itab)
...
*        output =
 ...
 ENDMETHOD.
  ...
  METHOD get_product_mrp BY DATABASE PROCEDURE FOR HDB LANGUAGE SQLSCRIPT
   ...
   lt_prod = select * from zbsh_ab_product;
   --count the records
...
BY DATABASE Function FOR HDB LANGUAGE SQLSCRIPT
   using zbsh_ab_bpa zbsh_ab_so_hdr.
   return select bpa.client,
          ...
          rank(  ) over ( order by sum( so.gross_amount ) desc )  as customer_rank
           from zbsh_ab_bpa as bpa
           inner join zbsh_ab_so_hdr as so
 ...
 METHOD execute BY DATABASE FUNCTION FOR HDB LANGUAGE SQLSCRIPT OPTIONS READ-ONLY.
